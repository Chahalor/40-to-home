...
/*   Updated: 2025/05/19 10:05:58 by nduvoid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mixed_test(void)
{
	...
	RUN_TEST(("%s %s %s", "Hello", "World", "!"), "3 strings");
	RUN_TEST(("%c %c %c", 'H', 'e', 'l'), "3 chars");
	RUN_TEST(("%d %d %d", 42, -42, 0), "3 ints");
	RUN_TEST(("%u %u %u", 42, -42, 0), "3 unsigned ints");
	RUN_TEST(("%x %x %x", 42, -42, 0), "3 hexadecimals");
	RUN_TEST(("%X %X %X", 42, -42, 0), "3 HEXADECIMALS");
	RUN_TEST(("dd %c %s %p %d %i %u %x %X %%",
		'Z', "(O-O)", (void *)42, 128, -74, -13, 66, 99), "mixed formats");
	RUN_TEST(("%  d", 42), "%   d %d with space");
}
...
// COLORS
#define ALLOW_COLOR 1

#if (defined (ALLOW_COLOR) && ALLOW_COLOR == 1)
