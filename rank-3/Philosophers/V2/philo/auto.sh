...
install:
	mkdir -p \$(INSTALL_DIR)
	cp \$(basename \$(NAME)) \$(INSTALL_DIR)/
	chmod +x \$(INSTALL_DIR)/\$(basename \$(NAME))
	\$(call _completion)
	echo "\033[1;32m ✅ \$(basename \$(NAME)) installed to \$(INSTALL_DIR) \033[0m"; \\

uninstall:
	rm -rf \$(INSTALL_DIR)/\$(basename \$(NAME))
	echo "\033[1;32m ✅ \$(basename \$(NAME)) uninstalled from \$(INSTALL_DIR) \033[0m";
