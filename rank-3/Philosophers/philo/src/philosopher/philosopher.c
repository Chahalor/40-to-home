...
/*   Updated: 2025/04/09 15:27:24 by nduvoid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (true)
	{
		...
		while (++i < global->data.nb_philo)
		{
			if (__builtin_expect(pthread_mutex_lock(global->philos[i]->lock), unexpected))
				return (NULL);
			else if (__builtin_expect(global->philos[i]->state == dead, unexpected))
				return (NULL);
			else if (__builtin_expect(time - global->philos[i]->last_meal
					> global->data.time_to_die, unexpected))
			{
				global->philos[i]->state = dead;
				return (info(dead, global->philos[i], time), NULL);
			}
			else if (__builtin_expect(pthread_mutex_unlock(global->philos[i]->lock),
				unexpected))
				return (NULL);
		}
	}
