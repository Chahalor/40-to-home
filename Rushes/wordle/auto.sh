...
while IFS= read -r -d '' dir; do
	MODULE_PATH="${dir#$DIR_SRC/}" # chemin relatif
	[[ -n "$MODULE_PATH" && "${MODULE_PATH}" != *"_internal" ]] && INCLUDE_ALL+=" -I\$(DIR_SRC)/$MODULE_PATH"
done < <(find "$DIR_SRC" -type d -print0)

# printf "INCLUDE_ALL: $INCLUDE_ALL\n"
...
for module in "$DIR_SRC"/*/; do
	if [ -d "$module" ]; then
		...
		# SRC_INTERFACE=$(find "$module" -maxdepth 2 -name "*.c" | sed 's|^.*/||' | tr '\n' ' ')	# listing all .c files in the module directory
		SRC_INTERFACE=$(find "$module" -maxdepth 3 -name "*.c" | sed 's|^[^/]*/||' | tr '\n' ' ')
		printf "  - SRC_INTERFACE: $SRC_INTERFACE\n"
		SRC_INTERNAL=""
...
OBJ_${MODULE_NAME^^}			:= \$(addprefix \$(DIR_OBJ)/, \$(SRC_INTERFACE_${MODULE_NAME^^}:.c=.o)) # \$(DIR_INTERFACE_${MODULE_NAME^^})/
OBJ_${MODULE_NAME^^}			+= \$(addprefix \$(DIR_OBJ)/, \$(SRC_INTERNAL_${MODULE_NAME^^}:.c=.o)) # \$(DIR_INTERNAL_${MODULE_NAME^^})/
